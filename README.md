...
docs/                     ← GitHub に公開するのはこのフォルダだけ
  ...
  assets/compat-copy.js   相性ページ専用の原稿（関係のなかでの立ち位置 / かみ合う形 /
                          崩れる入り口 / 用途ごとの読み解き）。ビルド時にだけ読む。
                          相性ページの生成文は types.js の軸テキスト36文が出どころで、
                          足すほど「全64枚に共通する文」が増えるだけなので、
                          固有性はここに手で書いたものでしか作れない。1コード1,400字が目安。
                          原稿の無いコードは従来の定型文に落ちる（16本ずつ足していける）。
...
| | 中身 | 出どころ |
|---|---|---|
| 版 | `1.1.0` のような3つの数字 | `package.json` の `version`（手で上げる） |
| ビルド番号 | `+` のあとの7文字 | `docs/` の中身のハッシュの頭7桁（自動） |
| 更新日 | `2026-09-01` | **中身が最後に変わった日**。ビルドを回した日ではない（自動） |

ビルド番号は、**問い合わせを受けたときにその人が見ているものを1つに特定する**ために出している。
版と日付だけだと、同じ日に2回直したときに区別がつかない。
`.ver` は `user-select:all` にしてあるので、利用者はひと押しで1行まるごと選べる。

| 上げるところ | いつ | 例 |
|---|---|---|
| メジャー `2.0.0` | **過去の結果とそのまま比べられなくなる**とき | 設問の入れ替え・増減、配点や軸の定義の変更、64タイプの分け方の変更、鑑定コードの形式変更 |
| マイナー `1.2.0` | 結果の数値は変わらないが、**できることや読めるものが変わる**とき | ページ・機能の追加や削除、解説文やタイプ名の書き換え |
| パッチ `1.1.1` | **読めるものは変わらない**とき | 見た目の調整、不具合の修正、文言の細かい直し |

`docs/assets/engine.js` の `QV`（設問の並びの版）か `CODE_VER`（鑑定コードの版）を
上げるときは、**必ずメジャーも上げる**。この2つは「過去と比べられるか」を決めている。

**中身を直したら、その内容と上の表を照らして版を判定する。**あとから思い出してやるものではない。
判定した結果が「据え置きでよい」なら、それも判定である。

`QV` と `CODE_VER` を動かしたのにメジャーを上げていないときだけは、`npm run stamp` が
ビルドを止める（この2つは「過去と比べられるか」を決めているので、見落とすと影響が大きい）。
マイナーとパッチの区別は判断なので自動化していない。上の表で決めること。

version を上げるのは手作業。ビルド番号と日付は `npm run stamp` が入れる。
`docs/` の中身のハッシュを `build-state.json` と突き合わせ、**変わっていなければ日付を動かさない**
（何も直していない日にビルドし直しても、更新日は前のまま）。
